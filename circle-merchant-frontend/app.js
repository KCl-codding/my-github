...
App({
  ...
  loadUnreadCountFromStorage() {
    try {
      const storageMap = wx.getStorageSync(this.getStorageKey('global_chat_unread_map')); 
  ...
  saveUnreadCountToStorage() {
    try {
      ...
      wx.setStorageSync(this.getStorageKey('global_chat_unread_map'), storageMap);
    } catch (e) {
  ...
  // 增加未读消息数
  addUnreadCount(openid) {
    const currentCount = this.globalData.unreadCountMap.get(openid) || 0;
    const newCount = currentCount + 1;
    this.globalData.unreadCountMap.set(openid, newCount);
    this.saveUnreadCountToStorage();

    // 立即更新角标
    this.updateTabBarBadge();

    console.log(`全局增加未读数: ${openid} = ${newCount}, 总未读数: ${this.getTotalUnreadCount()}`);
    return newCount;
  },

  // 清除指定聊天的未读消息数
  clearUnreadCount(openid) {
    console.log(`准备清除未读数: ${openid}`);
    console.log('清除前全局Map:', Array.from(this.globalData.unreadCountMap.entries()));

    if (this.globalData.unreadCountMap.has(openid)) {
      this.globalData.unreadCountMap.delete(openid);
      this.saveUnreadCountToStorage();

      // 立即更新角标
      this.updateTabBarBadge();

      console.log(`全局清除未读数成功: ${openid}`);
      console.log('清除后全局Map:', Array.from(this.globalData.unreadCountMap.entries()));
      console.log('清除后总未读数:', this.getTotalUnreadCount());
    } else {
      console.log(`未找到要清除的未读数: ${openid}`);
    }
  },
  ...
  updateTabBarBadge() {
    ...
    console.log('更新tabBar角标，总未读数:', totalUnread);

    // 获取当前页面路径
    const pages = getCurrentPages();
    const currentPage = pages[pages.length - 1];
    const isMessagePage = currentPage && currentPage.route === 'pages/message/message';

    // 如果在消息页面，不显示角标
    // if (isMessagePage) {
    //   console.log('当前在消息页面，隐藏角标');
  ...
  tryAutoLoginAndConnect() {
    const token = wx.getStorageSync(this.getStorageKey('token')); // ✅
    const expireTime = wx.getStorageSync(this.getStorageKey('tokenExpireTime')); // ✅
    const openId = wx.getStorageSync(this.getStorageKey('openId')); // ✅
    const userInfo = wx.getStorageSync(this.getStorageKey('userInfo')); // ✅
  ...
  // 清理登录数据
  clearLoginData() {
    wx.removeStorageSync(this.getStorageKey('token')); // ✅
  wx.removeStorageSync(this.getStorageKey('tokenExpireTime')); // ✅
  wx.removeStorageSync(this.getStorageKey('userInfo')); // ✅
  wx.removeStorageSync(this.getStorageKey('openId')); // ✅
  ...
  setupWebSocketAfterLogin(openId, token) {
    ...
    // 存储到本地
    wx.setStorageSync(this.getStorageKey('openId'), openId); 
  wx.setStorageSync(this.getStorageKey('token'), token);  
  ...
  initWebSocket(openId) {
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
    // const wsUrl = `wss://${API_CONFIG.DOMAIN}/ws/${openId}`;
  ...
  async pullOfflineMessages() {
    ...
    try {
      ...
      // 后端返回格式：{ "user_123": [msg1, msg2], "user_456": [msg3] }
